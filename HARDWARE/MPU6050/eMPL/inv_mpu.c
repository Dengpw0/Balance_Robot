...
#define i2c_write   MPU_Write_Len	//i2c_write函数由MPU_Write_Len函数实现
...
//返回值:0,正常
//    其他,失败
u8 mpu_dmp_get_data(float *pitch,float *roll,float *yaw)	//获取dmp计算角度
{
	...
	unsigned char more;
	long quat[4]; 
	if(dmp_read_fifo(gyro, accel, quat, &sensor_timestamp, &sensors,&more))	//如果读取到了fifo寄存器里的东西，会返回0，不会进入if语句
	return 1;	 //如果读取失败，return 1
	...
	**/
	if(sensors&INV_WXYZ_QUAT) //如果传感器收到数据
	{
